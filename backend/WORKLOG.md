...
## 🆕 NEW FEATURE: Conversational Context System

**Date:** 2026-01-14  
**Feature:** Chat History, Sidebar, and Context-Aware RAG  
**Status:** 🚧 In Progress
...
- [x] Design API contract for multi-turn conversations
- [x] Plan frontend sidebar and persistence architecture

#### Phase 2: Backend Implementation
- [ ] **Update Pydantic Model** (`main.py`)
  - Change `ChatRequest` from `message: str` to `messages: List[dict]`
  - Format: `[{"role": "user"|"assistant", "content": "..."}]`
  
- [ ] **Context Injection** (`way_rag/__init__.py`)
  - Modify `generate_answer()` to accept message history
  - Format history as: "User: ...\nAI: ..." in system prompt
  - Use last user message for vector search
  
- [ ] **Token Safety**
  - Limit context to last 6 messages
  - Prevents token limit issues with Groq API

#### Phase 3: Frontend Implementation
- [ ] **Create `useChatSessions` Hook**
  - Manage sessions array in state
...
- [ ] Context test: AI remembers name from previous messages
...
POST /api/chat
{
  "messages": [
    ...
    {"role": "assistant", "content": "สวัสดีครับคุณสมชาย"},
